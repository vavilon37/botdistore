# iPhone models from X (10) to 16 with variants, storage, colors
# Structure: model -> list of variants, each with storage and colors

IPHONE_MODELS = {
    "iPhone X": {
        "storage": ["64GB", "256GB"],
        "colors": ["Silver", "Space Gray"],
    },
    "iPhone XS": {
        "storage": ["64GB", "256GB", "512GB"],
        "colors": ["Silver", "Space Gray", "Gold"],
    },
    "iPhone XS Max": {
        "storage": ["64GB", "256GB", "512GB"],
        "colors": ["Silver", "Space Gray", "Gold"],
    },
    "iPhone XR": {
        "storage": ["64GB", "128GB", "256GB"],
        "colors": ["Black", "White", "Red", "Yellow", "Coral", "Blue"],
    },
    "iPhone 11": {
        "storage": ["64GB", "128GB", "256GB"],
        "colors": ["Black", "Green", "Yellow", "Purple", "Red", "White"],
    },
    "iPhone 11 Pro": {
        "storage": ["64GB", "256GB", "512GB"],
        "colors": ["Silver", "Space Gray", "Gold", "Midnight Green"],
    },
    "iPhone 11 Pro Max": {
        "storage": ["64GB", "256GB", "512GB"],
        "colors": ["Silver", "Space Gray", "Gold", "Midnight Green"],
    },
    "iPhone 12 mini": {
        "storage": ["64GB", "128GB", "256GB"],
        "colors": ["Black", "White", "Red", "Green", "Blue", "Purple"],
    },
    "iPhone 12": {
        "storage": ["64GB", "128GB", "256GB"],
        "colors": ["Black", "White", "Red", "Green", "Blue", "Purple"],
    },
    "iPhone 12 Pro": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Silver", "Graphite", "Gold", "Pacific Blue"],
    },
    "iPhone 12 Pro Max": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Silver", "Graphite", "Gold", "Pacific Blue"],
    },
    "iPhone 13 mini": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Red", "Starlight", "Midnight", "Blue", "Pink", "Green"],
    },
    "iPhone 13": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Starlight", "Midnight", "Blue", "Pink", "Green", "Red"],
    },
    "iPhone 13 Pro": {
        "storage": ["128GB", "256GB", "512GB", "1TB"],
        "colors": ["Silver", "Graphite", "Gold", "Sierra Blue", "Alpine Green"],
    },
    "iPhone 13 Pro Max": {
        "storage": ["128GB", "256GB", "512GB", "1TB"],
        "colors": ["Silver", "Graphite", "Gold", "Sierra Blue", "Alpine Green"],
    },
    "iPhone 14": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Midnight", "Purple", "Starlight", "Red", "Blue", "Yellow"],
    },
    "iPhone 14 Plus": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Midnight", "Purple", "Starlight", "Red", "Blue", "Yellow"],
    },
    "iPhone 14 Pro": {
        "storage": ["128GB", "256GB", "512GB", "1TB"],
        "colors": ["Space Black", "Silver", "Gold", "Deep Purple"],
    },
    "iPhone 14 Pro Max": {
        "storage": ["128GB", "256GB", "512GB", "1TB"],
        "colors": ["Space Black", "Silver", "Gold", "Deep Purple"],
    },
    "iPhone 15": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Black", "Pink", "Yellow", "Green", "Blue"],
    },
    "iPhone 15 Plus": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Black", "Pink", "Yellow", "Green", "Blue"],
    },
    "iPhone 15 Pro": {
        "storage": ["128GB", "256GB", "512GB", "1TB"],
        "colors": ["Black Titanium", "White Titanium", "Blue Titanium", "Natural Titanium"],
    },
    "iPhone 15 Pro Max": {
        "storage": ["256GB", "512GB", "1TB"],
        "colors": ["Black Titanium", "White Titanium", "Blue Titanium", "Natural Titanium"],
    },
    "iPhone 16": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Black", "White", "Pink", "Teal", "Ultramarine"],
    },
    "iPhone 16 Plus": {
        "storage": ["128GB", "256GB", "512GB"],
        "colors": ["Black", "White", "Pink", "Teal", "Ultramarine"],
    },
    "iPhone 16 Pro": {
        "storage": ["128GB", "256GB", "512GB", "1TB"],
        "colors": ["Black Titanium", "White Titanium", "Natural Titanium", "Desert Titanium"],
    },
    "iPhone 16 Pro Max": {
        "storage": ["256GB", "512GB", "1TB"],
        "colors": ["Black Titanium", "White Titanium", "Natural Titanium", "Desert Titanium"],
    },
    "iPhone 17": {
        "storage": ["256GB", "512GB"],
        "colors": ["Black", "White", "Mist Blue", "Sage", "Lavender"],
    },
    "iPhone Air": {
        "storage": ["256GB", "512GB", "1TB"],
        "colors": ["Space Black", "Cloud White", "Light Gold", "Sky Blue"],
    },
    "iPhone 17 Pro": {
        "storage": ["256GB", "512GB", "1TB"],
        "colors": ["Silver", "Cosmic Orange", "Deep Blue"],
    },
    "iPhone 17 Pro Max": {
        "storage": ["256GB", "512GB", "1TB", "2TB"],
        "colors": ["Silver", "Cosmic Orange", "Deep Blue"],
    },
}

# Groups for navigation: model number -> list of model names
IPHONE_GROUPS = {
    "iPhone X / XS / XR": ["iPhone X", "iPhone XS", "iPhone XS Max", "iPhone XR"],
    "iPhone 11": ["iPhone 11", "iPhone 11 Pro", "iPhone 11 Pro Max"],
    "iPhone 12": ["iPhone 12 mini", "iPhone 12", "iPhone 12 Pro", "iPhone 12 Pro Max"],
    "iPhone 13": ["iPhone 13 mini", "iPhone 13", "iPhone 13 Pro", "iPhone 13 Pro Max"],
    "iPhone 14": ["iPhone 14", "iPhone 14 Plus", "iPhone 14 Pro", "iPhone 14 Pro Max"],
    "iPhone 15": ["iPhone 15", "iPhone 15 Plus", "iPhone 15 Pro", "iPhone 15 Pro Max"],
    "iPhone 16": ["iPhone 16", "iPhone 16 Plus", "iPhone 16 Pro", "iPhone 16 Pro Max"],
    "iPhone 17": ["iPhone 17", "iPhone Air", "iPhone 17 Pro", "iPhone 17 Pro Max"],
}
